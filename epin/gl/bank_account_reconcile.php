...
$sql = "SELECT MAX(reconciled) as last_date,
		 SUM( case when reconciled<=to_date('$date','yyyy-mm-dd') then amount else 0 end) as end_balance,
		 SUM( case when reconciled<to_date('$date','yyyy-mm-dd') then amount else 0  end) as beg_balance,
		 SUM(amount) as total
