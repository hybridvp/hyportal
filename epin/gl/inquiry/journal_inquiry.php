...
$sql = "SELECT	decode(ISNULL(a.gl_seq),0,a.gl_seq) as gl_seq,
	gl.tran_date,
	gl.type,
	gl.type_no,
	refs.reference,
	...
	com.memo_,
	decode(ISNULL(u.user_id),'',u.user_id) as user_id
	FROM ".TB_PREF."gl_trans gl
	 LEFT JOIN ".TB_PREF."audit_trail as a ON 
		(gl.type=a.type AND gl.type_no=a.trans_no)
	 LEFT JOIN ".TB_PREF."comments as com ON 
		(gl.type=com.type AND gl.type_no=com.id)
	 LEFT JOIN ".TB_PREF."refs refs ON 
...
$sql = "SELECT	nvl(a.gl_seq,0) as gl_seq,
 	gl.tran_date,
 	gl.type,
 	gl.type_no,
 	refs.reference,
 	SUM( case when gl.amount>0 then  gl.amount else 0 end) as amount,
 	com.memo_,
 	...
 	FROM ".TB_PREF."gl_trans gl
 	 ...
 	 LEFT JOIN ".TB_PREF."users as u ON
 		a.user_=u.id
 	WHERE gl.tran_date >= to_date('" . date2sql($_POST['FromDate']) . "'
 	,'yyyy-mm-dd') AND gl.tran_date <= to_date('" . date2sql($_POST['ToDate']) . "'
 	,'yyyy-mm-dd') AND gl.amount!=0";
if (isset($_POST['Ref']) && $_POST['Ref'] != "") {
 	$sql .= " AND reference LIKE '%". $_POST['Ref'] . "%'";
}
if (get_post('filterType') != -1) {
...
$sql .= " GROUP BY gl.type, gl.type_no,a.gl_seq,gl.tran_date,refs.reference,com.memo_,u.user_id";
