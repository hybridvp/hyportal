...
function clear_data()
{
	...
	unset($_POST['no_sale']);
	unset($_POST['serialize']);
	unset($_POST['facevalue']);
}
 //-------------------------------------------------------------------------------------------
 function IsFaceValueExist($facevalue)
{
		$sql2 = "SELECT facevalue from "
			.TB_PREF."stock_master  
			WHERE facevalue=" . $facevalue;
			
	$sql_b = db_query($sql2);
	$result2 = db_fetch($sql_b);
	return $result2['facevalue'];
/* 	if ( $result2['facevalue']  != "" )
	   return true
	 else
	 	return false; ; */
}
...
if (isset($_POST['addupdate'])) 
{
	...
	if ($upload_file == 'No')
		$input_error = 1;
	if ( !ctype_alpha( substr($_POST['NewStockID'],1,1) ) ) 
	{
		display_error(_("The Item code must start with a letter."));
	...
	if (strlen($_POST['long_description']) == 0) 
	{
		$input_error = 1;
		display_error( _('The item description must be entered.'));
	...
	elseif (strlen($_POST['facevalue']) == 0 || input_num('facevalue') < 1) 
	{
		$input_error = 1;
		display_error( _('The face value cannot be empty'));
		set_focus('facevalue');
	}
	...
	elseif (!check_num('facevalue', 0))
	{
		$input_error = 1;
		display_error( _("The value entered must be numeric."));
		set_focus('facevalue');
	}
	if ($new_item) 
	{
		if(IsFaceValueExist(input_num('facevalue')) )
		{
			$input_error = 1;
			display_error( _("Item with facevalue ". $_POST['facevalue'] . " already exists"));
			set_focus('facevalue');
		}
	}
	...
	if ($input_error != 1)
	{
		...
		{ /*so its an existing one */
			update_item($_POST['NewStockID'], $_POST['description'],
				...
				check_value('no_sale'),check_value('serialize'),  $_POST['keepas'],input_num('facevalue') );
			...
			if($nonfin_audit_trail)
			{
				...
				add_nonfin_audit_trail(0,0,0,0,'ITEM MAINTENANCE','M',$ip,'ITEM CODE' . $_POST['NewStockID']. " MODIFIED ");
			}
		...
		{ //it is a NEW part
			...
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'ITEM MAINTENANCE','A',$ip,'ITEM CODE' . $_POST['NewStockID']. " ADDED ");
			}
		}
		$Ajax->activate('_page_body');
	}
...
function check_usage($stock_id, $dispmsg=true)
{
	...
	"SELECT COUNT(*) FROM "
		.TB_PREF."pin_details WHERE stock_id=".db_escape($stock_id)=>
		
	_('Cannot delete this item because there are existing EPINS for it.'),
	"SELECT COUNT(*) FROM "
...
if (isset($_POST['delete']) && strlen($_POST['delete']) > 1) 
{
	...
	if (check_usage($_POST['NewStockID'])) {
		...
		delete_item($stock_id);
			...
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'ITEM MAINTENANCE','D',$ip,'ITEM CODE' . $_POST['NewStockID']. " DELETED ");
			}
...
{ // Must be modifying an existing item
		...
		$_POST['serialize'] =  $myrow['serializable'];
		$_POST['del_image'] = 0;	
	 	$_POST['inactive'] = $myrow["inactive"];
		$_POST['keepas'] = $myrow["keep_inactive"];
		$_POST['facevalue'] = $myrow["facevalue"];
...
{
	small_amount_row(_("Face Value:"), 'facevalue', null, '', _(' '));
}
...
hidden ('sales_account', '4010');
hidden ('inventory_account', '1510');
hidden ('adjustment_account', '5010');
hidden ('cogs_account', '5040');
hidden ('assembly_account', '5040');
...
record_status_list_row(_("Keep Item as:"), 'keepas');
end_outer_table(1);
div_end();
div_start('controls');
