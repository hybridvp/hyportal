...
//------------------------------------------------------------------------------------------------
    ...
    $sql = "SELECT trans.type, 
		...
		(case when (trans.type = ".ST_SUPPINVOICE." OR trans.type = ".ST_SUPPCREDIT.") AND trans.due_date < to_date('" . date2sql(Today()) . "','yyyy-mm-dd') then 1 else 0 end) AS OverDue,
    	(case when ABS(trans.ov_amount + trans.ov_gst  + trans.ov_discount - trans.alloc) <= 0.005 then 1 else 0 end) AS Settled
    	FROM ".TB_PREF."supp_trans trans, ".TB_PREF."suppliers supplier
    	WHERE supplier.supplier_id = trans.supplier_id
     	AND trans.tran_date >= to_date('$date_after'". ",'yyyy-mm-dd')
    	AND trans.tran_date <= to_date('$date_to'".",'yyyy-mm-dd')
