...
if (isset($_GET['AddedID'])) 
{
	...
	//hyperlink_no_params($path_to_root . "/simplex/purchasing/inquiry/pr_confirm.php",_("Convert Another Purchase Requisition &To Order"));

	//http://localhost/frontaccount/simplex/purchasing/po_receive_serialize.php?TransID=109
	///hyperlink_params($path_to_root . "/purchasing/po_receive_items.php", _("&Receive Items on this Purchase Order"), "PONumber=$order_no");
//hyperlink_params($path_to_root . "/simplex/purchasing/po_receive_serialize.php", _("&Receive Items on this Purchase Order"), "TransID=$order_no");
	hyperlink_params($_SERVER['PHP_SELF'], _("Enter &Another Purchase Order"), "NewOrder=yes");
	
	hyperlink_no_params($path_to_root."/purchasing/po_search.php", _("Select An &Outstanding Purchase Order"));
...
function can_commit()
{
	...
	if (!$_SESSION['PO']->order_no) 
	{
    	...
    	if (!is_new_reference(get_post('ref'), ST_PURCHORDER)) 
    	{
			...
			$_SESSION['PO']->reference = $_POST['ref'] ;
			display_error(_("The entered reference is already in use, new reference has been obtained, try again now."));
			set_focus('ref');
    		return false;
    	}
	}
...
function handle_commit_order()
{
	...
	if (can_commit())
	{
		...
		if ($_SESSION['PO']->order_no == 0)
		{ 
		    //display_notification ($_SESSION['PO']->req_no) ;
			/*its a new order to be inserted */
			$order_no = add_po($_SESSION['PO']);
			new_doc_date($_SESSION['PO']->orig_order_date); 
		    if  ($_SESSION['PO']->requisition_no>0) {
			update_prpo($_SESSION['PO']->requisition_no);}
...
if (isset($_POST['CancelUpdate']))
	unset_form_variables();
////////////////Creating PO from Pr start from here
if (isset($_GET['ConvertOrder']) && $_GET['ConvertOrder'] != "")
{
	create_new_po();
//pr order number to be used to fetch the data for the PO to be created
	$_SESSION['PO']->order_no = $_GET['ConvertOrder'];	

	/*read in all the selected order into the Items cart  */
	read_prpo($_SESSION['PO']->order_no, $_SESSION['PO']);
	
	copy_from_cart();
	$_SESSION['PO']->requisition_no = $_SESSION['PO']->order_no ;
...
if (isset($_GET['ConvertOrder'])) $_POST['PRNO']=$_GET['ConvertOrder'];
if ($_SESSION['PO']->order_has_items()) 
{
	if ($_SESSION['PO']->order_no && !(isset($_GET['ConvertOrder'])) )
...
{	 	//$_SESSION['PO']->order_no = 0;
		submit_center_first('Commit', _("Place Order"), '', 'default');
	    submit_center_last('CancelOrder', _("Cancel Order")); 	
} else 	submit_center('CancelOrder', _("Cancel Order"), true, false, 'cancel');
}	
div_end();
//---------------------------------------------------------------------------------------------------
