...
page(_($help_context = "Manufacturers"), @$_REQUEST['popup']);
...
if (isset($_POST['submit'])) 
{
	...
	if (strlen($_POST['bank_account']) == 0 || $_POST['bank_account'] == "") 
	{
		$input_error = 1;
		display_error(_("The bank account name must be entered."));
		set_focus('bank_account');
	}
	if (strlen($_POST['supp_address']) == 0 || $_POST['supp_address'] == "") 
	{
		$input_error = 1;
		display_error(_("The address must be entered."));
		set_focus('supp_address');
	}
	if (strlen($_POST['phone']) == 0 || $_POST['phone'] == "") 
	{
		$input_error = 1;
		display_error(_("The phone  must be entered."));
		set_focus('phone');
	}
	...
	if ($input_error !=1 )
	{
		...
		else 
		{
			...
			$sql = "INSERT INTO ".TB_PREF."suppliers (supplier_id, supp_name, supp_ref, address, supp_address, phone, phone2, fax, gst_no, email, website,
				...
				VALUES (".db_escape($_POST['supplier_code']). ", ".db_escape($_POST['supp_name']). ", "
				.db_escape($_POST['supp_ref']). ", nvl("
				.db_escape($_POST['address']) . ",' '),nvl( "
				.db_escape($_POST['supp_address']) . ",' '), "
				.db_escape($_POST['phone']). ", nvl("
				.db_escape($_POST['phone2']). ",' '),nvl( "
				.db_escape($_POST['fax']). ",' '),nvl( "
				.db_escape($_POST['gst_no']). ",' '),nvl( "
				.db_escape($_POST['email']). ",' '),nvl( "
				.db_escape($_POST['website']). ",' '),nvl( "
				.db_escape($_POST['contact']). ",' '),nvl( "
				.db_escape($_POST['supp_account_no']). ",' '), nvl("
				...
				.db_escape($_POST['payment_discount_account']). ",nvl( "
				.db_escape($_POST['notes']). ",' '), "
				.db_escape($_POST['tax_group_id']). ")";

			db_query($sql,"The supplier could not be added");
			$_POST['supplier_id'] = $_POST['supplier_code'] ; //db_insert_id;
			$new_supplier = false;
...
if (!$new_supplier) 
{
	...
	$_POST['supplier_code'] = $myrow["supplier_id"];
...
{
	$_POST['supplier_code'] =$_POST['supp_name'] = $_POST['supp_ref'] = $_POST['address'] = $_POST['supp_address'] = 
...
text_row(_("Supplier Code:"), 'supplier_code', null, 20, 20);
