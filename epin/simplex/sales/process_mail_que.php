...
switch($SEND)
	{
		case "Send":
			if(!empty($MESSAGE))
			{
				...
				$HEADERS .= "From: Subject to be added <Sender@Blabla.com>\r\n";//Sender's address
	
				$linkID = mysql_connect("localhost","root","");//MySQL connection
				mysql_select_db(Database, $linkID);//Database name
	
				$obRS = mysql_query("SELECT Attribute(Email) FROM Table", $linkID);//Where the mail attribute
