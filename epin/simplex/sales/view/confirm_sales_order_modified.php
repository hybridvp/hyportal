...
if ($current_state != 'Planned')
  ...
  if (isset($_POST['ConfirmOrder'])) 
    { 
	if (confirm_sales_order ($customer_id, $order_no, $trans_type, $amount, $current_state)) 
	{
	     display_notification_centered(sprintf( _("Order # %d has been confirmed."),$order_no));
	}
        else {display_error( _("Order #".$order_no. " is mutating: could not be confirmed now, please review and retry."));}
     }
   else 
   {
	if (cancel_sales_order ($order_no, $trans_type, $amount, $current_state)) 
	{
        ...
        else {display_error( _("Order #".$order_no. " is mutating: could not be cancelled now, please review and retry."));}
...
if ($_GET['trans_type'] != ST_SALESQUOTE)
{
	...
	while ($credits_row = db_fetch($result))
	{
		...
		$this_total = $credits_row["ov_freight"] + $credits_row["ov_freight_tax"]  + $credits_row["ov_gst"] 
