...
{
	$_POST['line_no'] = $_GET['line_no'];
}

if (isset($_GET['jobid']))
{
	$_POST['jobid'] = $_GET['jobid'];
}
//-----------------------------------------------------------------------------------
// Ajax updates
//
...
$sql = "SELECT detail.order_no as order_no , batch_no,act.start_serial,act.end_serial,dat_logged,cod_user_id,
decode(job_status,'L','IN PROGRESS','C','COMPLETED',  'X','CANCELLED') status,
decode(action_mode,'104','ACTIVATION',105,'DEACTIVATION') status1, to_date( uvc_submit_time,'YYYYMMDDHH24:MI:SS') uvc_submit_time, to_date(uvc_finish_time,'YYYYMMDDHH24:MI:SS') uvc_finish_time, err.error_desc
		FROM "
		.TB_PREF."pin_mailer_jobs master, "
		.TB_PREF."pin_mailer_jobs_detail detail, "
		.TB_PREF."activation_jobs act, "
		.TB_PREF."epin_error_msg err
		 WHERE 1=1 
		 AND master.order_no = detail.order_no
		 AND master.line_no = detail.line_no
		 and detail.jobid = act.jobid
		 and detail.start_serial = act.start_serial
		 and detail.end_serial = act.end_serial
...
{
	$sql .= " AND act.jobid =".db_escape($jobid);
...
if ( isset($_GET['jobid'] ))
{
	$sql .= " AND act.jobid =".db_escape($_GET['jobid']);
}
...
$cols = array(
		...
		_("Status") ,
		_("Mode") ,
		_("Activation Submission Time") ,
		_("Activation Finish Time"), 
		_("Status Message") 
