...
page(_($help_context = "Authorize E-PIN File"), false, false, "", $js);
...
function approve_link($row) 
{
  //submit_center_first('ApproveOrder', $row, _('Approve Order'), 'default');
	...
	submit_cells('Authorise', _("Authorise"), "colspan=2",  _('Authorise this file'), true);
	submit_js_confirm('Authorise', _('You are about to Authorise this file.\nDo you want to continue?'));
}
...
function approve_line($file_number)
{
	//$result = GetTmpPin($file_number);
	//begin_transaction();
/*	
			
	$sql = "UPDATE ".TB_PREF."pin_details SET flg_mnt_status = 'A',authorised_by = " .db_escape($_SESSION["wa_current_user"]->loginname) . " WHERE batch_no= " . 
			$file_number ." and flg_mnt_status = 'U'";
			db_query($sql, "The order could not be authorised.");
			*/
	display_notification ("Document approved");
		$ip = preg_quote($_SERVER['REMOTE_ADDR']);
	add_nonfin_audit_trail(0,0,0,0,'DATAFILE IMPORT AUTHORISATION','A',$ip,'DATAFILE BATCH NO:' . $file_number . " AUTHORISED ");
	//Display_error ("Document fully authorised already Ponumber=" . $ponumber . "id=" . $trans_id); 
	unset ($_SESSION['file_number']);
	
}
function cancel_line($file_number)
{
	//$result = GetTmpPin($file_number);
	//begin_transaction();
	...
	display_notification ("Document approved");
	$ip = preg_quote($_SERVER['REMOTE_ADDR']);
	add_nonfin_audit_trail(0,0,0,0,'DATAFILE IMPORT CANCEL','A',$ip,'DATAFILE BATCH NO:' . $file_number . " CANCELLED ");
	//Display_error ("Document fully authorised already Ponumber=" . $ponumber . "id=" . $trans_id); 
	unset ($_SESSION['file_number']);
}
...
else
{
 ...
 if (isset($_POST['Authorise']))
  {
  		approve_line($_SESSION['file_number'] ) ;
  }
   if (isset($_POST['Cancel']))
  {
  		cancel_line($_SESSION['file_number'] ) ;
  }
  
//figure out the sql required from the inputs available //stock.order_no,
//substr(suppliers.supp_name,1,2)
/* $sql = "SELECT 
	 batch_no, denomination,  decode(status,'N','NEW','S', 'SOLD','D','DELIVERED' ) status, trunc(tmp.load_date),sales_order_no, count(*) ,created_by 
	FROM "
		.TB_PREF."pin_details tmp
	WHERE tmp.flg_mnt_status= 'U'"; */
$sql = " SELECT batch_no,facevalue,decode(status,'A','AUTHORISED','B','BLACKLISTED','D','DELIVERED','N','NEW','S', 'SOLD', 'U','USED') status , 
trunc(load_date),quantity,'' created_by,b.error_desc FROM "  .TB_PREF."pin_master tmp, ". TB_PREF. "epin_error_msg b WHERE tmp.error_code = b.error_code (+) and status = 'N'";
...
//$sql .= "group by  batch_no, denomination, status, trunc(tmp.load_date),sales_order_no, created_by";
//echo $sql;
