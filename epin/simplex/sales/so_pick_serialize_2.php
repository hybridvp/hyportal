...
if (isset($_GET['AddedID']))
{
	...
	hyperlink_params("$path_to_root/simplex/sales/inquiry/sales_orders_pick.php", _("Select a different &sales order for picking"));
...
if ((!isset($_GET['OrderNumber']) || $_GET['OrderNumber'] == 0)  && !isset( $_POST['OrderNum']))
{
	die (_("This page can only be opened if a sales order # has been selected. Please select a sales order #  first."));
}
if(isset($_GET['OrderNumber']) )
...
function display_so_serialize_items()
{
	...
	$line_qty = $_SESSION['remainder'] ; //$result["quantity"] ;  //- $result["stock.qty_serialized"]
	$remainder  = $line_qty;
...
function process_serialize_so()
{
	...
	display_notification(_('Items Successfuly picked'));
	...
	meta_forward($_SERVER['PHP_SELF'], "AddedID=".$_POST['OrderNum']);
...
function setvariables()
{
	...
	$cart['OrderNumber'] = $_POST['OrderNum'];
...
if (isset($_POST['ScanCard']))
{
	...
	header("Location:".$path_to_root. "/simplex/sales/so_pick_serialize_3.php?OrderNumber=" . $_POST['OrderNum']);
}
...
//hidden('OrderNum', $_GET['OrderNumber']);
if (isset($_GET['OrderNumber']))
{
	hidden('OrderNum', $_GET['OrderNumber']);
}
else
	hidden('OrderNum', $_POST['OrderNum']);
