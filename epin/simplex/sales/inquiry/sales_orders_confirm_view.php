...
{
	$strQuery = "select role_id from users where upper(user_id) = " . strtoupper(db_escape($username) );
...
function edit_link($row)
{
 ...
 return pager_link( _("Confirm Sales Order"),
	"/simplex/sales/view/confirm_sales_order.php?trans_no=" .$row['order_no']. "&trans_type=".$row['trans_type'], ICON_EDIT);
...
//		AND debtor.debtor_no = branch.debtor_no
		"AND sorder.ourorder_status = 'CLOSED' " ;
		//AND sorder.trans_type = appr.trans_type 
		//AND sorder.order_no = appr.order_no
		//AND appr.trans_type = 30 ";
		//AND u.role_id = (select approving_role from sales_approval where sales_type= 30)
		//AND upper(u.user_id) = upper(" . db_escape($_SESSION["wa_current_user"]->loginname) .")";
