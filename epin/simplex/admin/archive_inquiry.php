...
page(_($help_context = "Search Archived EPINS"), false, false, "", $js);
...
$sql = "SELECT batch_no, sequence_number,  denomination,load_date,  created_by,sales_order_no,customer_no,
			   decode(status,'A','AUTHORISED','B','BLACKLISTED','D','DELIVERED','N','NEW','S', 'SOLD', 'U','USED') status,  decode(flg_mnt_status,'A', 'Authorised', 'U', 'Unauthorised')
		FROM "
		.TB_PREF."PIN_DETAILS_MMDD epin "; 
		
if (isset($_GET['stat'])){
	$sql .= " WHERE flg_mnt_status=". db_escape(strtoupper ($_GET['stat']));
}
else
...
if($nonfin_audit_trail)
			...
			add_nonfin_audit_trail(0,0,0,0,'EPIN ARCHIVE INQUIRY','I',$ip,'INQUIRY DONE ON THIS RECORD - ');
			}
