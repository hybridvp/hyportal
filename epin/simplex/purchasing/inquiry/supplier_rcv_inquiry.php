...
function change_tpl_flag($id)
{
	...
	$pin_status = get_trans_stat ($id) ;
  	$sql = "UPDATE ".TB_PREF."rcv_mailer_jobs SET status='Q' WHERE order_no =$id and '" . $pin_status . "'= 'A'";
...
$sql = "SELECT epin.order_no, epin.line_no, detail.pin_qty, denomination,file_gen_date,  logged_by,epin.supplier_no,decode(epin.status,'L','IN PROGRESS','S', 'STOPPED','C','ACTIVATING','M','DELIVERED', 'Q','QUEUED','A','ACTIVATED', 'F', 'FAILED') status, detail.file_name ||'.pgp.gz' as filename	,  epin.status as  queued	,jobid   
		FROM "
		.TB_PREF."rcv_mailer_jobs epin ,"
		.TB_PREF."rcv_mailer_jobs_detail detail 
...
$result = db_query($sql,"No items were returned");
		...
		if($nonfin_audit_trail)
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'SUPPLIER RCV INQUIRY','A',$ip,'INQUIRY DONE ON THIS RECORD');
			}
//echo $sql;
