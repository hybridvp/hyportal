...
page(_($help_context = "Search serialized Items"), false, false, "", $js);
...
$sql = "SELECT 
	...
	AND smaster.serializable=1 AND status = 'AVAILABLE'";
	//AND qty > 0 ";
...
{
	$sql .= " AND serialized.order_no LIKE ".db_escape('%'. $order_number . '%');
}
else
{
