...
if (isset($_GET['AddedID']))
{
	...
	display_notification_centered(_("Purchase Order #" . $_GET['AddedID'] . "Serialized successfully"));

	//display_note(get_trans_view_str($trans_type, $grn, _("&View this Delivery")));
	
	hyperlink_params("$path_to_root/simplex/purchasing/inquiry/scanned_item_inquiry.php" , _("&View this scan"), "order_number=". $_GET['AddedID']);

	hyperlink_no_params("$path_to_root/simplex/purchasing/inquiry/received_item_search.php", _("Select a different &purchase order for serializing"));
	...
	if($remainder > 0 ) //&&  $_POST['units'] == 'bx.'
		{
			display_notification_centered(_("Cannot scan as selected Unit , Not enough items"));
		//"/$path_to_root .simplex/purchasing/po_receive_serialize_2.php?TransID=".$TransID . "&Rem="
			hyperlink_no_params( "#", _("Cannot scan as selected Unit , Not enough items"));
			//submit_cells('ScanBrick', _("Next"), "colspan=2", _('Scan remaining items as Bricks'), true);
		}	
	if ( isset($_SESSION['remainder']) && $_SESSION['remainder'] <=0 )
		{
...
function process_serialize_po()
{
	...
	$location = $cart[1];
	$unit = $cart[2];
	//Validate that entered serials have been loaded before
/*	foreach($all_serials as $row)
	{  
		if( !serial_loaded($row,$location,$unit,$result['stock_id']) )
		{
			hyperlink_back();
			display_error(_( " Item with serial ". $row . " does not exist in the selected location ".$_REQUEST['FromLocation']));
			return ;
		}
	}*/
	begin_transaction();
...
if (isset($_GET['TransID'] ) )//|| isset($_POST['transidhidden']) )
{
	hidden('transidhidden', $_GET['TransID']);
}
else
	hidden('transidhidden', $_POST['transidhidden']);
