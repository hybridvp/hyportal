...
page(_($help_context = "RCV Activation Request"), false, false, "", $js);
...
if (isset($_GET['AddedID']))
{
	...
	display_notification("Activation job " . $_GET['AddedID'] ." was successfully Queued");
...
function AddRange ($batch_no="",$start_no="", $end_no="", $action_mode)
{
	...
	$sql = "INSERT INTO ".TB_PREF."ACTIVATION_JOBS
			...
			VALUES ($id,". db_escape($batch_no) . "," .db_escape($start_no) . "," .db_escape($end_no). ", sysdate,'L',". db_escape($action_mode). ",". db_escape($_SESSION["wa_current_user"]->loginname) . ", "   . db_escape($job_id)  .", 'RCV', 2)";  //to_char(sysdate,'YYYYMMDDHH24MISS')" 
							db_query($sql, "Could not insert rcv activation job");
...
{
		$sql2 = "SELECT count(1) from "
			.TB_PREF."rcv_pin_details  
			WHERE sequence_number=" . db_escape($sequence_no);
			
	$sql_b = db_query($sql2, "select from rcv_pin_details failed");
	$row = db_fetch_row($sql_b);
	return ($row[0] > 0);
}
	...
	if ($input_error != 1)
	{
		...
		//{
			...
			if($nonfin_audit_trail)
			{
				...
				add_nonfin_audit_trail(0,0,0,0,'RCV ACTIVATION REQUEST','A',$ip,'REQUEST NUMBER ' . $addedid. " SUBMITTED ");
			}
...
table_section_title(_("Activation Request"));
