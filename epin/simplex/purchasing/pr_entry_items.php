...
/////////////////////////////////////////
$path_to_root = "../..";
include_once($path_to_root . "/simplex/purchasing/includes/pr_class.inc");
include_once($path_to_root . "/includes/session.inc");
include_once($path_to_root . "/simplex/purchasing/includes/requisition_ui.inc");
...
{
	page(_($help_context = "Modify Requisition #") . $_GET['ModifyOrderNumber'], false, false, "", $js);
} 
else 
{
	page(_($help_context = "Purchase Requisition"), false, false, "", $js);
}
...
if (isset($_GET['AddedID'])) 
{
	...
	display_note(get_trans_view_str($trans_type, $order_no, _("&View this requisition")), 0, 1);

	display_note(print_document_link($order_no, _("&Print This requisition"), true, $trans_type), 0, 1);

	display_note(print_document_link($order_no, _("&Email This requisition"), true, $trans_type, false, "", "", 1));

	//These are going to change later
...
function copy_from_cart()
{
	...
	$_POST['approved_by1'] = $_SESSION['PO']->approved_by1;	
...
function copy_to_cart()
{
	...
	$_SESSION['PO']->approved_by1 = $_POST['approved_by1'];
}
//--------------------------------------------------------------------------------------------------
...
function unset_form_variables() {
	...
	unset($_POST['approved_by1']);
}

//---------------------------------------------------------------------------------------------------

function start_approval($pr_no)
{
		$sql = "UPDATE ".TB_PREF."purch_reqs SET status='Pending'
		WHERE pr_no =".db_escape($pr_no);

		db_query($sql, "The purchase requistion could not be updated.");
...
function handle_delete_item($line_no)
{
	...
	{
		display_error(_("This item cannot be deleted because some of it has already been converted to PO lines."));
	}	
    line_start_focus();
}
...
function handle_cancel_pr()
{
	...
	if($_SESSION['PO']->order_no != 0)
	{
		delete_pr($_SESSION['PO']->order_no);
	} else {
		unset($_SESSION['PO']);
	...
	display_notification(_("This purchase requisition has been cancelled."));

	hyperlink_params($path_to_root . "/simplex/purchasing/pr_entry_items.php", _("Enter a new purchase requisition"), "NewOrder=Yes");
	echo "<br>";
...
function handle_update_item()
{
	...
	if ($allow_update)
	{
		...
		$_SESSION['PO']->update_order_item($_POST['line_no'], $_POST['costcentre'], input_num('qty'), input_num('price'),
  			$_POST['req_del_date']);
		unset_form_variables();
	}	
    line_start_focus();
			...
			if ($allow_update)
		   	{
				...
				$_SESSION['PO']->add_to_order ($_POST['line_no'], $_POST['stock_id'], input_num('qty'), 
					$myrow["description"], $_POST['costcentre'],input_num('price'), $myrow["units"],
...
function can_commit()
{
	...
	{
    	if (!$OurRefs->is_valid(get_post('ref'))) 
    	{
...
function handle_commit_order()
{
	...
	if (can_commit())
	{
		...
		{ 
			...
			$order_no = update_pr($_SESSION['PO']);
...
if (isset($_GET['ForwardOrderNumber']) && $_GET['ForwardOrderNumber'] != "")
{
///Update status to Pending
start_approval($_GET['ForwardOrderNumber']); 

	display_notification(_("The purchase requisition has been forwarded to the authoriser."));

	hyperlink_no_params($path_to_root . "/simplex/purchasing/inquiry/pr_search_completed.php", _("Forward another purchase requisition"));
	echo "<br>";
...
if (isset($_GET['ModifyOrderNumber']) && $_GET['ModifyOrderNumber'] != "")
{
	...
	read_pr($_SESSION['PO']->order_no, $_SESSION['PO']);
...
display_pr_header($_SESSION['PO']);
echo "<br>";

display_pr_items($_SESSION['PO']);
...
{
	if ($_SESSION['PO']->order_no)
		submit_center_first('Commit', _("Update Requisition"), '', 'default');
	else
		submit_center_first('Commit', _("Save Requisition"), '', 'default');
	submit_center_last('CancelOrder', _("Cancel Requisition")); 	
}
else
	submit_center('CancelOrder', _("Cancel Requisition"), true, false, 'cancel');
div_end();
//---------------------------------------------------------------------------------------------------
