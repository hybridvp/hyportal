...
function display_po_serialize_items()
{
	...
	$line_qty = $_SESSION['remainder2'] ; //$result["qty"] ;  //- $result["stock.qty_serialized"]
	$remainder  = $line_qty;
...
function db_serialize($cart,$serials)
{
	...
	$strSQL .= ",order_no,trans_date) values (SERIALIZED_STOCKS_ID_SEQ.NEXTVAL,null," . db_escape($result['stock_id']) . ",". db_escape($location) . "," . $cart[3] . "," ."null,". db_escape($row) . ","  . $result['order_no'] . ",sysdate)";
...
{
	//if( !isset($_SESSION['CART2']) || !isset($_SESSION['CART3']))
	//	return;
