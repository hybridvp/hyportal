...
// save also in hidden field for testing during 'Update'
 ...
 return checkbox(null, $name, $value, true,
 	_('Set this order for Re-encryption and Resale'))
...
function change_tpl_flag($id)
{
	global	$Ajax;
  	...
  	$sql = "UPDATE ".TB_PREF."pin_mailer_jobs_detail SET status = 'F' WHERE order_no=$id ";//AND line_no=$line

  	db_query($sql, "Can't set status for pin resale");
	
	$sql = "UPDATE ".TB_PREF."pin_mailer_jobs SET status = 'Q' WHERE order_no=$id ";//AND line_no=$line
  	db_query($sql, "Can't set status for pin resale");
...
//Added: commented out the conversion of SQ to SO as it is now a separate function 					
	 ...
	 array_append($cols,array(
			_("Resell") => array('insert'=>true, 'fun'=>'tmpl_checkbox'),
					//array('insert'=>true, 'fun'=>'edit_link'),
					array('insert'=>true, 'fun'=>'prt_link')));
};
