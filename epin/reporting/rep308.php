...
cod_key1_value,cod_key2_value
	...
	$sql = 'BEGIN pkg_inventory_movement.prc_inventory_statement(:vstart_date,:vend_date, :sales_summary); END;';
...
function print_audit_trail()
{
    ...
    $cols = array(0, 70, 160, 340, 400, 450 , 510); //array(0, 60, 120, 180, 300, 120 , 120, 60, 120, 120);

    $headers = array(_('Tran Date'), _('DP Code'), _('DP Name'),
    	 _('Invoice No'), _('Product'), _('Quantity'));
    ...
    $rep = new FrontReport(_('Detailed Transaction Report'), "DetailedTransactionReport", user_pagesize());
    $rep->Font();
    $rep->Info($params, $cols, $headers, $aligns);
    $rep->Header();
//    $trans = getTransactions($from, $to);
	$sql = 'BEGIN pkg_inventory_movement.prc_sales_detail(:vstart_date,:vend_date,:sales_detail); END;';
							...
							// Create a new cursor resource
							$sales_detail = oci_new_cursor($db);
							
							// Bind the cursor resource to the Oracle argument
							oci_bind_by_name($stmt,":sales_detail",$sales_detail,-1,OCI_B_CURSOR);							
							// Execute the statement
							oci_execute($stmt);
							
							// Execute the cursor
							oci_execute($sales_detail);

    while ($myrow=db_fetch($sales_detail))
    {
        $rep->TextCol(0, 1, $myrow['order_date']);
        //if (user_date_format() == 0)
        //	$rep->TextCol(1, 2, $myrow['logged_date']);
        //else	
        //	$rep->TextCol(1, 2, $myrow['logged_date']);
		$rep->TextCol(1, 2, $myrow['dealer_id']);
        $rep->TextCol(2, 3, $myrow['dealer']);
		$rep->TextCol(3, 4, $myrow['invoice_no']);
        $rep->TextCol(4, 5, $myrow['facevalue']);
