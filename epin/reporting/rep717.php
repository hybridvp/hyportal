...
if($nonfin_audit_trail)
			...
			add_nonfin_audit_trail(0,0,0,0,'REPORT PEQUEST','A',$ip,'REPORT 717 REQUESTED ');
}
...
//to_char(dat_txn,'dd/mm/yyyy') ,to_char(dat_txn,'HH24:MM:SS')
	...
	$sql = "select to_char(dat_txn,'dd/mm/yyyy') dat_txn, to_char(dat_txn,'HH24:MM:SS') time_txn, 
	cod_key2 msisdn, cod_key2_value pin, cod_key1_value srl, cod_user_id, cod_key1 amt, cod_trans_type, 
	decode(cod_mnt_action,'A' ,'ADD','D','DELETE','I','INQUIRY') action

		FROM ".TB_PREF."audit_trail_nonfin a ,"
		      .TB_PREF."users u 
		WHERE 1=1 
		AND cod_trans_type ='PIN RECHARGE'
...
function print_audit_trail()
{
    ...
    $cols = array(0, 60, 120, 190, 250, 370 , 430, 500, 560);

    $headers = array(_('Date'), _('Time'), _('MSISDN'), _('PIN'), _('SERIAL'), _('Amount'), _('User'),
    	_('Trans Type')  );
    ...
    $rep = new FrontReport(_('PIN Recharge Activity Report'), "PINRecharge", user_pagesize());
    ...
    while ($myrow=db_fetch($trans))
    {
        ...
        if (user_date_format() == 0)
        	...
        	$rep->TextCol(1, 2, $myrow['time_txn']);
        $rep->TextCol(2, 3, $myrow['msisdn']);
//        $rep->TextCol(3, 4, $systypes_array($myrow['cod_trans_type']));
		$rep->TextCol(3, 4, $myrow['pin']);
        $rep->TextCol(4, 5, $myrow['srl']);
        $rep->TextCol(5, 6, str_replace("\\",".",$myrow['amt']) );
        //if ($myrow['gl_seq'] == null)
       // 	$action = _('Changed');
       // else
       // 	$action = _('Closed');
//        $rep->TextCol(6, 7, $myrow['action']);
		$rep->TextCol(6, 7, $myrow['cod_user_id']);
		$rep->TextCol(7, 8, $myrow['cod_trans_type']);
