...
if($nonfin_audit_trail)
			...
			add_nonfin_audit_trail(0,0,0,0,'REPORT PEQUEST','A',$ip,'REPORT 714 REQUESTED ');
}
...
cod_key1_value,cod_key2_value
	...
	$sql = "select a.order_no,a.customer_name,quantity,file_gen_date,denomination ,b.file_name,b.start_serial,b.end_serial
...
function print_audit_trail()
{
    ...
    $cols = array(0, 60, 150, 190, 250, 310 , 370, 450, 500);
	
    $headers = array( _('Order #'), 
    	 _('Customer name'), _('Quantity'), _('Start Serial'), _('End Serial'),  _('Date') _('File'), _('Value'));
    ...
    $rep = new FrontReport(_('Customer EPIN Sales'), "Customer EPIN Sales", user_pagesize());
    ...
    while ($myrow=db_fetch($trans))
    {
		...
		$rep->TextCol(3, 4, $myrow['start_serial']);
        $rep->TextCol(4, 5, $myrow['end_serial']);
		
		
		/* if (user_date_format() == 0)
        	$rep->TextCol(5, 6, $myrow['file_gen_date']);
        else */
			$rep->TextCol(5, 6, $myrow['file_gen_date']);
