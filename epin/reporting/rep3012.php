...
function getTransactions__($category, $location)
{
	/*$sql = "SELECT ".TB_PREF."stock_mastr.category_id,
			".TB_PREF."stock_category.description AS cat_description,
			".TB_PREF."stock_master.stock_id,
			".TB_PREF."stock_master.description,
			".TB_PREF."pin_details.location,
		...
		//if ($location != 'all')
			$sql .= TB_PREF."pin_details.location, ";
			$sql .= TB_PREF."stock_master.facevalue
		
			".TB_PREF."stock_master.stock_id,
			".TB_PREF."stock_master.description
		HAVING SUM(denomination/denomination) != 0";
		if ($category != 0)
			$sql .= " AND ".TB_PREF."stock_master.category_id = ".db_escape($category);
		if ($location != 'all')
			$sql .= " AND ".TB_PREF."pin_details.location = ".db_escape($location);
		$sql .= " ORDER BY ".TB_PREF."stock_master.category_id,
            ...
            pin_details.location,
            SUM(denomination/denomination) AS QtyOnHand,
            stock_master.facevalue AS unitcost,
            SUM(denomination/denomination)  * (stock_master.facevalue) As ItemTotal
        FROM stock_master,
            stock_category,
            pin_details
        WHERE stock_master.stock_id=pin_details.stock_id
        AND stock_master.category_id=stock_category.category_id
        GROUP BY stock_master.category_id,
            stock_category.description, pin_details.location, stock_master.facevalue,
            stock_master.stock_id,
            stock_master.description,
            pin_details.denomination
        HAVING SUM(denomination/denomination) != 0 ORDER BY stock_master.category_id,
            stock_master.stock_id  "
