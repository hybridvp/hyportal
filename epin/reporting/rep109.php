...
function print_sales_orders()
{
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		while ($myrow2=db_fetch($result))
		{
			...
			//Added to show Taxes
			if ($myrow2["tax_rate"]!=0)
			{
				if ($tax=="1") $tax = $myrow2["tax_name"]." (".$myrow2["tax_rate"]."%)";
				else if ($tax != $myrow2["tax_name"]." (".$myrow2["tax_rate"]."%)") //another tax type should be added to the string
				   			$tax .= ", ".$myrow2["tax_name"]." (".$myrow2["tax_rate"]."%)";
			$tax_value += $myrow2["tax_rate"]*$Net/100 ; 
			}	
			//
        ...
        //$DisplayTax = ; 
		$rep->NewLine();
		$rep->TextCol(3, 6, $tax, -2);
		$rep->TextCol(6, 7,	number_format2($tax_value, $dec), -2);
		//
		$rep->NewLine();
		//Modified by adding tax value to the total to be displayed
		$DisplayTotal = number_format2($myrow["freight_cost"] + $SubTotal+$tax_value, $dec);
		$rep->Font('bold');
		if ($tax!="1") $rep->TextCol(3, 6, "TOTAL ORDER, ".$tax." INCL.", - 2);
		else
