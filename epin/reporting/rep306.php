...
function print_audit_trail()
{
    ...
    $cols = array(0, 70, 160, 240, 320, 390 , 460); //array(0, 60, 120, 180, 300, 120 , 120, 60, 120, 120);

    $headers = array(_('Stock ID'), _('Opening Stock'), _('Closing Stock'),
    	 _('Qty Loaded'), _('Qty Sold'), _('Qty Released'));
    ...
    $rep = new FrontReport(_('Inventory Movement Report'), "InvMovementReport", user_pagesize());
    $rep->Font();
    $rep->Info($params, $cols, $headers, $aligns);
    $rep->Header();
//    $trans = getTransactions($from, $to);
	$sql = 'BEGIN pkg_inventory_movement.prc_inventory_statement(:vstart_date,:vend_date,:sales_inventory); END;';
    ...
    while ($myrow=db_fetch($sales_inventory))
    {
        ...
        //else	
        //	$rep->TextCol(1, 2, $myrow['logged_date']);
		$rep->TextCol(1, 2, $myrow['opening_stock']);
        $rep->TextCol(2, 3, $myrow['closing_stock']);
		$rep->TextCol(3, 4, $myrow['qty_loaded']);
        $rep->TextCol(4, 5, $myrow['qty_sold']);
        $rep->TextCol(5, 6, $myrow['qty_released']);
