...
//--------------------------------------------------------------------------------------------------
function reference_exist($ref)
{
		$sql2 = "SELECT reference from "
			.TB_PREF."sales_orders  
			WHERE reference = ". db_escape($ref);
			
	$sql_b = db_query($sql2);
	$result2 = db_fetch($sql_b);
	if ( $result2['reference'] == $ref)
		return true;
	else
		return false;
	//return $result2['name'];
...
if (isset($_GET['AddedID'])) {
	$order_no = $_GET['AddedID'];
		//$url = print_document_link($order_no, "&Email This Order", true, ST_SALESORDER, false, 'menu_option', null, 1, 0);
	...
	//print_document_link($order_no, "&Email This Order to the Customer", true, ST_SALESORDER, false, 'menu_option', null, 1, 0);
	//submenu_print(_("&Email This Order to the Dealer"), ST_SALESORDER, $order_no, null, 1);
	...
	//$sender,$recipient,$bcc,$cc,$attach,$status,$log_date,$url)


	//echo $url;
	set_focus('prtopt');

//Added: Commented out delivery option after order entry, next step is confirmation 
//	submenu_option(_("Make &Delivery Against This Order"),
//		"/sales/customer_delivery.php?OrderNumber=$order_no");
//Added: Commented out this as new order cannot be entered directly on the system with the new flow/requirements.
//	submenu_option(_("Enter a &New Order"),	"/sales/sales_order_entry.php?NewOrder=0");
	...
	submenu_print(_("&Email This Quotation"), ST_SALESQUOTE, $order_no, null, 1);
	set_focus('prtopt');
//Added: Commented out as this is now a separate function to be performed by someone else
//	submenu_option(_("Make &Sales Order Against This Quotation"),
//		"/sales/sales_order_entry.php?NewQuoteToSalesOrder=$order_no");
...
/* 	if (!isset($_POST['approved_by1'])) {
		display_error(_("You must select an approving officer."));
		set_focus('approved_by1');
		return false;
	} */
	...
	//if (!$Refs->reference_exist( $_POST['ref'])) {
	if ($Refs->exists( ST_SALESORDER, $_POST['ref'])) {
		display_error(_("You must enter a unique reference."));
		set_focus('ref');
		return false;
	}
	return true;
...
if (isset($_POST['ProcessOrder']) && can_process()) {
	...
	add_sales_tran_approval($trans_no,$_SESSION["wa_current_user"]->loginname, $_POST['approved_by1'],ST_SALESORDER);
	// Send email
	//$body = "You Order Number ". $order_no . " is being processed" ;
	$myrow = get_company_prefs();
	$body = $myrow["msg_body"];
	//echo " body of mail = " .  $body . "<br>"; 
	$subject = $myrow["msg_subject"];
	//get_doc_link($order_no, "&Email This Order", true, ST_SALESORDER, false, 'menu_option', null, 1, 0);
	
	
	//$body = get_company_pref("msg_body");
	//$subject = get_company_pref("msg_subject");
	if (isset($_POST['approved_by1'] ))
	{
		sendmail(ST_SALESORDER, $body, $_POST['approved_by1'], $trans_no, $subject);//$order_no
	}
	if($trans_type == ST_SALESQUOTE)
	{
			...
			add_nonfin_audit_trail(0,0,0,0,'SALES QUOTATION ENTRY','A',$ip,'SALES QUOTATION NUMBER ' . $trans_no. " ADDED ");
			}
	}
	...
	if($nonfin_audit_trail)
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'SALES ORDER ENTRY','A',$ip,'SALES ORDER NUMBER ' . $trans_no. " ADDED ");
			}
	}
	elseif ($trans_type == ST_SALESINVOICE) {
		if($nonfin_audit_trail)
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'SALES INVOICE ENTRY','A',$ip,'SALES INVOICE NUMBER ' . $trans_no. " ADDED ");
			}
		...
		is_inventory_item($_POST['stock_id']))
	{
		...
		if (input_num('qty') > $qoh)
		{
			...
			display_error(_("The delivery cannot be processed because there is an insufficient quantity for item:") .
				...
				_("Quantity On Hand") . " = " . number_format2($qoh, get_qty_dec($_POST['stock_id'])) . " on " . $_POST['OrderDate']);
			return false;
		}
		return true;
	}
...
if ($customer_error == "") {
	...
	if ($_SESSION['Items']->trans_no == 0) {
		submit_center('ProcessOrder', $porder,
		    _('Check entered data and save document'), 'default');
		submit_js_confirm('CancelOrder', _('You are about to void this Document.\nDo you want to continue?'));
	} else {
		//submit_center('ProcessOrder', $corder,
		 //   _('Validate changes and update document'), 'default');
		//Changed the commit button to save changes in sales_order_entry.php

		$order_no = 0 ;
		//$quote_no = 0 ;
		$trans_type	= 0 ;
		
		//if (isset($_GET['ModifyQuotationNumber'])) $quote_no =  $_GET['ModifyQuotationNumber'] ;
		if (isset($_GET['trans_type'])) $trans_type	 = $_GET['trans_type'] ; 
		$modify = ($trans_type == ST_SALESORDER ? "ModifyOrderNumber" : "ModifyQuotationNumber");
		if (isset($_GET[$modify])) $order_no = $_GET[$modify] ;
        //if ($quote_no>0 && $order_no==0) $order_no = $quote_no ;
		 
   	    if (get_order_state($order_no, $trans_type)=="Planned") 
