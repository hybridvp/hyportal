...
// Commented out delivery option after order entry, next step is confirmation 
...
if ($customer_error == "") {
    ...
    if (isset($_GET['ModifyOrderNumber']))
	 {
	 $current_status = get_order_state($_REQUEST['ModifyOrderNumber'], $_REQUEST['trans_type']);
	 }
	else     
	{   if (isset($_GET['ModifyQuotationNumber'])) 
		{$current_status = get_order_state($_REQUEST['ModifyQuotationNumber'], $_REQUEST['trans_type']);}
	}
echo 'cur stat:' .$current_status;
    if (trim ($current_status)=="Planned") 
	    {
			display_order_summary($orderitems, $_SESSION['Items'], true);
		}
	else 
		display_order_summary($orderitems, $_SESSION['Items'], false);
