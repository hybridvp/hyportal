...
function get_job_header($order_no)
{
	$sql = "SELECT order_no,line_no,decode(status,'L','IN PROGRESS','S', 'STOPPED','C','ACTIVATING','M','DELIVERED', 'Q','QUEUED','A','ACTIVATED', 'F', 'FAILED') status FROM ".TB_PREF."pin_mailer_jobs where order_no =". db_escape($order_no) ;
    $result = db_query($sql, "Can not look up jobs in pin_mailer_jobs");
    $row = db_fetch_row_r($result);
    return $row;
}
function get_job_detail($order_no, $line_no)
{
	$sql = "SELECT a.order_no, a.line_no, decode(a.status,'L','IN PROGRESS','S', 'STOPPED','C','ACTIVATING','M','DELIVERED', 'Q','QUEUED','A','ACTIVATED', 'F', 'FAILED', 'D', 'DEACTIVATED') status,
	b.start_serial, b.end_serial, file_gen_date dispatch_time,
			job_status, to_date( uvc_submit_time,'YYYYMMDDHH24:MI:SS') uvc_submit_time,  to_date( uvc_start_time,'YYYYMMDDHH24:MI:SS') uvc_start_time ,
			to_date( uvc_finish_time,'YYYYMMDDHH24:MI:SS') uvc_finish_time, d.error_desc, e.time_delivered
			FROM "
			.TB_PREF."pin_mailer_jobs a, 
			".TB_PREF."pin_mailer_jobs_detail b, 
			".TB_PREF."activation_jobs c,
			".TB_PREF."epin_error_msg d,
			".TB_PREF."sales_orders e
			WHERE a.order_no = b.order_no
			AND a.line_no = b.line_no
			AND b.jobid= c.jobid
			AND e.order_no = a.order_no
			AND c.error_code = d.error_code(+)
			AND a.order_no= ". $order_no .
			" AND a.line_no =". $line_no;
    $result = db_query($sql, "Can not look up pin_mailer_jobs_detail");
    $row = db_fetch_row_r($result);
    return $row;
}
...
//added to transfer status to the next form for checking when to make the form editable
//$_SESSION['ourorder_status']= ourorder_status;
...
//$myrow = get_job_header($_POST['trans_no']) ;
//label_row(_("File Status"), $myrow['2'], "class='tableheader2'", "colspan=3");
end_table();

if ($_GET['trans_type'] != ST_SALESQUOTE)
{
//disable Deliveries and INvoices
...
$th = array(_("Item Code"), _("Item Desc"), _("Qty"), _("Unit"),
	_("Qty Dispatched"),_("Dispatch Time"), _("Activation Start Time"), _("Activation End Time") , _("Time Delivered") ,_("File Status") , _("Job Status"));  //_("Price"), _("Discount"), _("Total"), 
table_header($th);
...
foreach ($_SESSION['View']->line_items as $stock_item) {
	...
	$myrow = get_job_detail($_POST['trans_no'], $stock_item->id) ;
...
//print_r($_SESSION['View']->line_items); 
