...
function detail_link($row)
{
	global $trans_type;
	return  get_customer_trans_view_detail($trans_type, $row['order_no']);
}
...
function dispatch_link($row)
{
	...
	if ($trans_type == ST_SALESORDER)
  		return pager_link( _("Dispatch"),
			"/sales/customer_delivery.php?OrderNumber=" .$row['order_no'] . "&jobflg=". $_GET['jobflg'], ICON_DOC);
	else
  		return pager_link( _("Sales Order"),
			"/sales/sales_order_entry.php?OrderNumber=" .$row['order_no'] . "&jobflg=". $_GET['jobflg'], ICON_DOC);
}
...
else	// ... or select inquiry constraints
{
  	if ($_POST['order_view_mode']!='DeliveryTemplates' && $_POST['order_view_mode']!='InvoiceTemplates')
  	{
		...
		$sql .=  " AND trunc(sorder.ord_date) >= '$date_after'"
				." AND trunc(sorder.ord_date) <= '$date_before'";
  	}
	...
	//Added status check for delivery  in sales_orders_view.php
		$sql .= " AND line.qty_sent < line.quantity  " ; //AND ourorder_status= 'CLOSED'";
...
if ($trans_type == ST_SALESORDER)
	$cols = array(
		...
		_("Approved By"),
		_("Approval Date"),
		//_("Order Total") => array('type'=>'amount', 'ord'=>''),'Type' => 'skip',
		_("Delivery Date"),
...
//Added: commented out the conversion of SQ to SO as it is now a separate function 					
	 ...
	 array_append($cols,array(
			_("Details") => array('insert'=>true, 'fun'=>'detail_link'),					
					array('insert'=>true, 'fun'=>'prt_link')));
};
//array('insert'=>true, 'fun'=>'edit_link'),
