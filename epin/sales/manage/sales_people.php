...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if (strlen($_POST['salesman_phone']) == 0)
	{
		$input_error = 1;
		display_error(_("The sales person phone number cannot be empty."));
		set_focus('salesman_phone');
	}
	if (strlen($_POST['salesman_email']) == 0)
	{
		$input_error = 1;
		display_error(_("The sales person email address cannot be empty."));
		set_focus('salesman_email');
	}
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
				...
				if($nonfin_audit_trail)
			{
				...
				add_nonfin_audit_trail(0,0,0,0,'SALES PERSON MNT','M',$ip,'SALES PERSON' . $_POST['salesman_name']. " MODIFIED");
			}
    	}
    	else
    	{
    		/*Selected group is null cos no item selected on first time round so must be adding a record must be submitting new entries in the new Sales-person form */
    		$sql = "INSERT INTO ".TB_PREF."salesman (salesman_code,salesman_name, salesman_phone, salesman_fax, salesman_email,
    			provision, break_pt, provision2)
    			VALUES (SALESMAN_CODE_SEQ.nextval,".db_escape($_POST['salesman_name']) . ", "
				  .db_escape($_POST['salesman_phone']) . ", nvl("
				  .db_escape($_POST['salesman_fax']) . ",'080'), "
			...
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'SALES PERSON MNT','A',$ip,'SALES PERSON' . $_POST['salesman_name']. " ADDED ");
			}
    	}
...
if ($Mode == 'Delete')
{
	...
	else
	{
		...
		db_query($sql,"The sales-person could not be deleted");
			...
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'SALES PERSONS MNT','D',$ip,'SALES PERSON' .$selected_id. " DELETED");
			}
