...
page(_($help_context = "Dealer Branches"), @$_REQUEST['popup']);

include($path_to_root . "/includes/ui.inc");

//-----------------------------------------------------------------------------------------------

check_db_has_customers(_("There are no dealers defined in the system. Please define a dealer to add dealer branches."));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM')
{
	...
	{
		$input_error = 1;
		display_error(_("The Branch Phone Number cannot be empty."));
		set_focus('phone');
	}
	if (strlen($_POST['email']) == 0)
	{
		$input_error = 1;
		display_error(_("The Branch Email Address cannot be empty."));
		set_focus('email');
	}
	if (strlen($_POST['br_address']) == 0)
	{
		$input_error = 1;
		display_error(_("The Branch Address cannot be empty."));
		set_focus('phone');
	}
	if ($input_error != 1)
	{
    	...
    	if ($selected_id != -1)
		{
			...
			$sql = "UPDATE ".TB_PREF."cust_branch SET br_name = " . db_escape($_POST['br_name']) . ",
    	        ...
    	        phone=nvl(".db_escape($_POST['phone']). ",' '),
    	        phone2=nvl(".db_escape($_POST['phone2']). ",' '),
    	        fax=nvl(".db_escape($_POST['fax']).",' '),
    	        contact_name=".db_escape($_POST['contact_name']) . ",
    	        salesman= ".db_escape($_POST['salesman']) . ",
    	        area=".db_escape($_POST['area']) . ",
    	        email=nvl(".db_escape($_POST['email']) . ",' '),
    	        ...
    	        default_location=nvl(".db_escape($_POST['default_location']) . ",'DEF'),
                ...
                notes=nvl(".db_escape($_POST['notes']) . ",' ')
    	        WHERE branch_code =".db_escape($_POST['branch_code']) . "
    	        AND debtor_no=".db_escape($_POST['customer_id']);

			$note =_('Selected dealer branch has been updated');
			if($nonfin_audit_trail)
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'DEALER BRN MNT','M',$ip,'DEALER BRN' . $_POST['branch_code']. " MODIFIED");
			}
		}
		else
		{
			/*Selected branch is null cos no item selected on first time round so must be adding a	record must be submitting new entries in the new Customer Branches form */
			$db_ins_id = db_insert_id("cust_branch_code_seq");
			...
			$sql = "INSERT INTO ".TB_PREF."cust_branch (branch_code, debtor_no, br_name, branch_ref, br_address,
				...
				VALUES (".db_escape($db_ins_id). ",".db_escape($_POST['customer_id']). ",".db_escape($_POST['br_name']) . ", "
					.db_escape($_POST['br_ref']) . ", "
					.db_escape($_POST['br_address']) . ", ".db_escape($_POST['salesman']) . ",nvl( "
					.db_escape($_POST['phone']) . ",' '), nvl(".db_escape($_POST['phone2']) . ",' '), nvl("
					.db_escape($_POST['fax']) . ",' '),"
					.db_escape($_POST['contact_name']) . ", ".db_escape($_POST['area']) . ",nvl("
					...
					.db_escape($_POST['sales_discount_account']) . ", nvl("
					.db_escape($_POST['default_location']) . ", 'DEF') ,"
					.db_escape($_POST['br_post_address']) . ","
					.db_escape($_POST['disable_trans']) . ", "
					.db_escape($_POST['group_no']) . ", "
					.db_escape($_POST['default_ship_via']). ",nvl( "
					.db_escape($_POST['notes']) . ", ' '))";
			
			$note = _('New dealer branch has been added');
			if($nonfin_audit_trail)
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'DEALER BRN MNT','A',$ip,'DEALER BRN' . $db_ins_id. " ADDED");
			}
		}
...
elseif ($Mode == 'Delete')
{
	...
	if ($myrow[0] > 0)
	{
		display_error(_("Cannot delete this branch because dealer transactions have been created to this branch."));
	...
	else
	{
		...
		else
		{
			...
			display_notification(_('Selected dealer branch has been deleted'));
			if($nonfin_audit_trail)
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'DEALER BRN MNT','D',$ip,'DEALER BRN' .$_POST['branch_code']. " DELETED");
			}
		}
...
$num_branches = db_customer_has_branches($_POST['customer_id']);
	...
	if (!get_post('show_inactive')) $sql .= " AND b.inactive=0";
	//echo $sql;
//------------------------------------------------------------------------------------------------
if ($num_branches)
{
...
display_db_pager($table);
	...
	display_note(_("The selected dealer does not have any branches. Please create at least one branch."));
...
sales_region_list_row( _("Sales Region:"), 'region', null, false);
