...
if (isset($_GET['jobflg'])) {
	$_POST['jobflg'] = $_GET['jobflg'] ;
}
...
//Added checks to see that the order to be delivered has been confirmed in customer_delivery.php to function check_data() 
   /*      if (get_order_state($_SESSION['Items']->order_no) != 'CLOSED') //22-feb-2012
	{
...
///////////////////////////////
	...
	if ($_SESSION['Items']->trans_no==0) {
		...
		/* if ($_SESSION['Items']->trans_no==0 && !is_new_reference($_POST['ref'], ST_CUSTDELIVERY)) {
			display_error(_("The entered reference is already in use. ". $_POST['ref']));
			set_focus('ref');
			return false;
		} */
		/* if ($_SESSION['Items']->trans_no==0 && !is_new_reference($_POST['ref'], ST_CUSTDELIVERY)) {
			...
			if ($itm->qty_dispatched && has_stock_holding($itm->mb_flag)) {
				...
				if ($itm->qty_dispatched > $qoh) {
					display_error(_("The delivery cannot be processed because there is an insufficient s quantity for item:") .
						" " . $itm->stock_id . " - " .  $itm->item_description .  " QOH = " . $qoh);
					return false;
				}
			}
		}
...
$sql = "SELECT qty_in_stock FROM ".TB_PREF."denom_sequence_control where item_type= 'EPIN' and denomination= (select facevalue from stock_master where stock_id= ". db_escape($stock_id) . ")" ;
...
function get_reference_no($order_no)
{
	$sql = "SELECT reference FROM ".TB_PREF."sales_orders where order_no =". db_escape($order_no) ;
    $result = db_query($sql, "Can not look up reference number");
    $row = db_fetch_row_r($result);
    return $row[0];
}
...
function dispatch_to_cust()
{
		foreach ($_SESSION['Items']->line_items as $line=>$ln_itm) {
			...
			if( $ln_itm->qty_dispatched > $qpoh ){
				display_error(_("The delivery cannot be processed because there are insufficient PINs for item:") . " " . $ln_itm->stock_id . 					" - " .  $ln_itm->item_description . 'qpoh =' . $qpoh) ;
				return false;
				}
			...
			//$myrow = get_company_prefs();
			 ...
			 // Send email
			//$body = get_doc_link($_SESSION['Items']->order_no, "&Email This Order", true, ST_CUSTDELIVERY, false, 'menu_option', null, 1, 0);
//			$body = "You Order Number ". $_SESSION['Items']->order_no . " is being processed" ;
			//$myrow["delivery_body"]
			$body = get_company_pref("delivery_msg_body");
			$subject = get_company_pref("delivery_msg_subject");
			//sendmail(ST_CUSTDELIVERY, $body, $_SESSION["wa_current_user"]->loginname,$_SESSION['Items']->order_no, $subject);  // sales order approval; = 90
					if($return) 
						return false;
			//}
		}
	dispatch_sales_tran_approval($_SESSION['Items']->order_no,$_SESSION["wa_current_user"]->loginname,$_SESSION["wa_current_user"]->loginname,ST_CUSTDELIVERY);
		return true;
}
...
//	if($jobflag
	...
	$sql = "INSERT INTO ".TB_PREF."pin_mailer_jobs	(id,order_no,line_no,customer_no,customer_name,quantity,stock_id,
				delivery_address,contact_phone,contact_email,logged_date,logged_by, status,filename,denomination,sold_from_loc,job_flag, item_type)
				...
				", ". $facevalue. "," .  db_escape($_POST['Location']) . ",". db_escape($jobflag).", 'EPIN')";  //$_POST['jobflg']
...
if (isset($_POST['process_delivery']) && check_data() && check_qoh() && dispatch_to_cust()) {
	...
	//$current_state =  get_order_state($_GET['OrderNumber']); //   $_SESSION['Items']->order_no);
	$trans_type = ST_SALESORDER;
	//secure sales order for confirmation 
  	//change_order_state ($_SESSION['Items']->order_no, $trans_type,$current_state, 'ChangingState' ) ;
	...
	change_order_state ($_SESSION['Items']->order_no, $trans_type, 'Confirmed' ,'Dispatched') ;  //$_SESSION['Items']->order_no
	processing_end();
			if($nonfin_audit_trail)
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'SALES ORDER DELIVERY','A',$ip,'DELIVERY # ' . $delivery_no. " DELIVERED ");
			}
	if ($newdelivery) {
...
$reference = get_reference_no($_SESSION['Items']->order_no);
if ($_SESSION['Items']->trans_no==0) {
label_cells(_("Invoice Number"),    $reference,       "class='tableheader2'");
	//ref_cells(_("Invoice Number"), 'ref', '', null, "class='tableheader2'");
	//text_cells(_("Invoice number").':', 'ref', '',16, 18, _('Invoice number unique for this document type'), null, '');
	hidden("ref",$reference);
	
	
} else {
	//label_cells(_("Invocie Number"), $_SESSION['Items']->reference, "class='tableheader2'");
	label_cells(_("Invocie Number"), $reference, "class='tableheader2'");
...
$current_state = get_order_state($_SESSION['Items']->order_no);
if( $current_state == 'Dispatched')
{
	display_error(_("Order #".$_SESSION['Items']->order_no. " not in suitable state for modification. Status is " . $current_state));
	end_form();
	end_page();
	exit();
...
foreach ($_SESSION['Items']->line_items as $line=>$ln_itm) {
	...
	view_stock_status_cell($ln_itm->stock_id); //delivered
	...
	//small_qty_cells(null, 'Line'.$line, qty_format($ln_itm->qty_dispatched, $ln_itm->stock_id, $dec), null, null, $dec);//this delivery
	hidden('Line'.$line,qty_format($ln_itm->qty_dispatched, $ln_itm->stock_id, $dec)  );  //this delivery
	label_cell(qty_format($ln_itm->qty_dispatched, $ln_itm->stock_id, $dec));
