...
//				 	error_log('invalid area id: '.$a.':'.$id);
			...
			$new_ids[$role_id] = db_insert_id("security_roles_id_seq");
			import_security_role($new_ids[$role_id], $pref, $security_headings[$role_id], $sections, $area_set);
...
function import_security_role($id, $pref, $name, $sections, $areas)
{
    //use this security_roles_id_seq
	$sql = "INSERT INTO {$pref}security_roles (id, role, description, sections, areas)
	VALUES (".db_escape($id).",".db_escape('FA 2.1 '.$name).",".db_escape($name).","
...
function sanitize_database($pref, $test = false) {
	 ...
	 $tsql = "SELECT table_name from user_tables where table_name LIKE '%".($pref=='' ? '' : substr($pref,0,-1).'\\_')."%'";
	 $tresult = db_query($tsql, "Cannot select all tables with prefix '$pref'");
	 while($tbl = db_fetch($tresult)) {
		$table = $tbl[0];
		$csql = "select COLUMN_NAME FROM  user_tab_cols where table_name = $table";
