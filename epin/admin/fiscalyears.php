...
function delete_this_fiscalyear($selected_id)
{
	...
	while ($row = db_fetch($result))
	{
		...
		if (is_account_balancesheet($row['account']))
		{
			...
			$counter = db_insert_id ("gl_trans_counter_seq") ;
			$sql = "INSERT INTO ".TB_PREF."gl_trans (counter, type, type_no, tran_date, account, memo_, amount) VALUES
				(".$counter.", ".ST_JOURNAL.", $trans_no, '$to', '{$row['account']}', '$ref', {$row['amount']})";
	...
	while ($row = db_fetch($result))
	{
		...
		$sql = "INSERT INTO ".TB_PREF."bank_trans (id, type, trans_no, trans_date, bank_act, ref, amount) VALUES
			(BANK_TRANS_ID_SEQ.nextval, 0, 0, '$to', '{$row['bank_act']}', '$ref', {$row['amount']})";
