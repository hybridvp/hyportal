...
$system_tests = array('tst_orcl', 'tst_php', 'tst_server', 'tst_system', 'tst_browser',
...
{
	$strQuery = "select version from dba_registry where comp_id = 'ORDIM'";
	$result = db_query($strQuery,"No rows were returned");
	return db_fetch($result) ;
}
function tst_mysql() 
{
	$test['descr'] = _('MySQL version'). ' >3.23.58';
	//$test['descr'] = _('Oracle version'). ' >= 9.2.0.7';
	$test['type'] = 3;
...
function tst_orcl() 
{
	$test['descr'] = _('Oracle version'). ' >= 9.2.0.7';
	$test['type'] = 3;
	$test['test'] = oracle_version(); //mysql_get_server_info();
	$test['result'] = $test['test']>'3.23.58';
	$test['comments'] =  _('Upgrade Oracle Database to version at least 9.2.0.7') ;//_('Upgrade MySQL server to version at least 3.23.58');
