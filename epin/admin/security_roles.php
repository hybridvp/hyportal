     	...
     	if ($new_role) 
       	{
			...
			if($nonfin_audit_trail)
			{
				...
				add_nonfin_audit_trail(0,0,0,0,'ROLES MAINTENANCE','A',$ip,'ROLE ' . $_POST['role']. " ADDED ");
			}
       	...
       	} else
       	{
			...
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'ROLES MAINTENANCE','M',$ip,'ROLE ' . $_POST['role']. " MODIFIED ");
			}
...
if (get_post('delete'))
{
	if (check_role_used(get_post('role'))) {
		...
		delete_security_role(get_post('role'));
			...
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'ROLES MAINTENANCE','D',$ip,'ROLE ' . get_post('role'). " DELETED ");
			}
