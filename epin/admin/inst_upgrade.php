...
{
	$tables = @db_query("SELECT table_name from user_tables where table_name LIKE '%".$pref.$table."%'");
	if (!db_num_rows($tables))
		return 1;		// no such table or error

	$fields = @db_query("select COLUMN_NAME FROM  user_tab_cols where table_name =".$pref.$table);
	if (!isset($field)) 
