...
function can_process() 
{
	...
	{
		display_error( _("You must enter the Full name."));
		set_focus('real_name');
		return false;
	}
	if (strlen($_POST['email']) < 1)
	{
		display_error( _("You must enter an email address ."));
		set_focus('email');
		return false;
	}
	...
	if (can_process())
	{
    	if ($selected_id != -1) 
    	{
    		update_user($selected_id, $_POST['user_id'], $_POST['real_name'], $_POST['phone'],
				...
				$_POST['profile'], check_value('rep_popup'), $_POST['pos'], $_POST['loc']);
			...
			if($nonfin_audit_trail)
			{
				...
				add_nonfin_audit_trail(0,0,0,0,'USER MAINTENANCE','M',$ip,'USER ' . $_POST['user_id']. " MODIFIED ");
			}
    	...
    	else 
    	{
			...
			$id = db_insert_id("users_id_seq");
    		add_user($id,$_POST['user_id'], $_POST['real_name'], md5($_POST['password']),
				$_POST['phone'], $_POST['email'], $_POST['Access'], $_POST['language'],
				$_POST['profile'], check_value('rep_popup'), $_POST['pos'], $_POST['loc']);
			if($nonfin_audit_trail)
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'USER MAINTENANCE','A',$ip,'USER ' . $_POST['user_id']. " ADDED ");
			}
			//$id = db_insert_id;
...
if ($Mode == 'Delete')
{
	delete_user($selected_id);
			...
			{
				$ip = preg_quote($_SERVER['REMOTE_ADDR']);
				add_nonfin_audit_trail(0,0,0,0,'USER MAINTENANCE','D',$ip,'USER ' . $selected_id. " DELETED ");
			}
...
if ($selected_id != -1) 
{
  	if ($Mode == 'Edit') {
		...
		$_POST['pos'] = $myrow["pos"];
		$_POST['loc'] = $myrow["default_location"];
	}
...
locations_list_row(_("User's Default Location"). ':', 'loc', null);
